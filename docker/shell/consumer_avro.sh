...
function sintaxe {
    ...
    echo ""    
    echo "Exemplo: ./consumer_avro.sh account-created test1 10000 earliest"
    echo ""
 }
...
if [ ! -z "$1" ] && [ ! -z "$2" ]; then
    ...
    if [ ! -f "$PROPERTIES" ]; then
        echo "Arquivo $PROPERTIES não encontrado!"
        sintaxe
        exit 1
    fi

    # Consumir mensagens avro

    kafka-avro-console-consumer --bootstrap-server $(prop 'bootstrap.servers') \
    --topic $1 \
    --timeout-ms $TIMEOUT \
